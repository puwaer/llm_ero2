...
base_config_path = os.path.dirname(sftlab_dir) + '/sftlab/base_config/base_config.yaml'
...
# wandb_project を exp_config から取得
wandb_project = exp_config.get('wandb_project', 'default_project')  
wandb_task = exp_config.get('wandb_task', 'default_name') 

# wandbの設定
wandb_entity = base_config['wandb']['entity']
if args.debug:
    wandb_project = 'sftlab-debug'  # デバッグモードでは固定
wandb_group = wandb_task
wandb_name = f'{wandb_task}'
...
# モデルの保存先（output_dir）を wandb_project に連動させる
if args.debug:
    output_dir = 'sftlab-debug'
else:
    output_dir = f'{sftlab_dir_name}/{exp_project_name}/{wandb_task}'  
output_dir = base_config['output_dir'] + '/' + output_dir  # base_config の output_dir と結合
