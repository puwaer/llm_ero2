    ...
    for data_config in data_config_list:
        if split in data_config['split']:
            ...
            def tokenize_function(example):
                tokenized = tokenizer(example[result_key], truncation=True, padding="max_length", max_length=4096)
                tokenized["labels"] = tokenized["input_ids"].copy()  # 自己回帰モデルのためlabelsをinput_idsと同じに
                return tokenized
            
            dataset = dataset.map(tokenize_function, batched=True, remove_columns=[result_key])
...
def main() -> None:
    ...
    trainer = SFTTrainer(
        model,
        args=training_args,
        processing_class=tokenizer,  # tokenizer -> processing_class に変更
